...
\begin{frame}
    ...
    \begin{algorithm}[H]
        ...
        \State{$\mathtt{return}\ [1]$}
        \EndIf
        \State{$\mathtt{let}\ resultado\gets [l]$}
        \For{$\mathtt{let}\ i=2$ {\bf upto} $l$}
        \State{$\mathtt{let}\ corte\gets [i]+\mathtt{Cortar}(l-i)$}
        \If{$\mathtt{Precio}(corte)>\mathtt{Precio}(resultado)$}
        \State{$resultado\gets corte$}
        \EndIf
        \EndFor
        \State{$\mathtt{return}\ resultado$}
        \EndProcedure
        \end{algorithmic}
    \end{algorithm}
